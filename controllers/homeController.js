...
const generatePdf = async (req, res) => {
  try {
    ...
    if (data.quotation === "new") {
      ...
      var config = {
        method: "post",
        url: `http://localhost:3000/api/quotation/${data.query_id}`,
        headers: {
    ...
    const filename =
      QuotationNo.split("/")[0] + "-" + QuotationNo.split("/")[1] + ".pdf";
...
const generateInvoicePdf = async (req, res) => {
  try {
    ...
    var config = {
      method: "post",
      url: `http://localhost:3000/api/invoice/createInvoice`,
      headers: {
        "Content-Type": "application/json",
        Cookie:
          "darshanSession=s%3A1pHuUrUxP4VvI_q9PGtz-E7QGHQYB0bC.zID6MNIzgEpXQ8LL%2FylJsR8NfLPG8OSl6NzjnCatxDE",
    ...
    const filename =
      InvoiceNo.split("/")[0] + "-" + InvoiceNo.split("/")[1] + ".pdf";
    ...
    if (details.metadata.currency === "INR") {
      if (details.client.client_state === "Gujarat") {
        ...
        CGST =
          ((subtotal -
            parseFloat(details.metadata.packaging_and_forwarding_charges) -
